...
puts inarray.zip(inarray.map { |item, _count| item[0..0].capitalize.to_s + item[-2..-1].to_s + "(#{inarray.index(item) + 1})" }).to_h
puts
...
puts "\xD1\x81\xD0\xBF\xD0\xB8\xD1\x81\xD0\xBE\xD0\xBA \xD1\x83\xD0\xBD\xD0\xB8\xD0\xBA\xD0\xB0\xD0\xBB\xD1\x8C\xD0\xBD\xD1\x8B\xD1\x85 \xD1\x81\xD0\xBB\xD0\xBE\xD0\xB2 \xD0\xBD\xD0\xB0 (\xD0\xB8) \xD0\xB2 \xD0\xBF\xD0\xBE\xD1\x80\xD1\x8F\xD0\xB4\xD0\xBA\xD0\xB5 \xD1\x83\xD0\xB2\xD0\xB5\xD0\xBB\xD0\xB8\xD1\x87\xD0\xB5\xD0\xBD\xD0\xB8\xD1\x8F \xD0\xBA\xD0\xBE\xD0\xBB\xD0\xB8\xD1\x87\xD0\xB5\xD1\x81\xD1\x82\xD0\xB2\xD0\xB0 \xD1\x81\xD0\xB8\xD0\xBC\xD0\xB2\xD0\xBE\xD0\xBB\xD0\xBE\xD0\xB2 \xD0\xB2 \xD1\x81\xD0\xBB\xD0\xBE\xD0\xB2\xD0\xB5"
taskdata = IO.read('task7.txt').encode!('UTF-8', 'UTF-8', invalid: :replace)
puts taskdata.scan(/[^,!*:?.";\n'\d\\{}()<=>\[\]%]/).join('').split(' ').select! { |elem| elem[0] == 'и' }.uniq!.sort_by(&:size).inject('') { |str, elm| str = str + elm.to_s + ' ' }
puts
...
def findfiles(dirfist = Dir.pwd)
  ...
  rbcount = 0
  list.each { |elm| rbcount += findfiles(dirfist + '/' + elm) if File.directory?(dirfist + '/' + elm) }
